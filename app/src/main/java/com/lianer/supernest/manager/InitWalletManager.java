...
//                    HLWalletManager.shared().saveWallet(context, hlWallet);
                    ...
                    KLog.i("InitWalletManager","keystore:\n" + keystore);
                    return hlWallet;
                });
    }
        ...
        return flowable
                .flatMap(s -> {
                    ...
                    if (HLWalletManager.shared().isWalletExist(hlWallet.getAddress())) {
                        return Flowable.error(new HLError(ReplyCode.walletExisted, new Throwable("该钱包已存在！")));
                    }
                    HLWalletManager.shared().saveWallet(context, hlWallet);
    ...
    public ECKeyPair generateKeyPair(String mnemonics) {
        ...
        KLog.i("InitWalletManager","mnemonics:" + mnemonics);
        String extendedBase58 = rootKey.extendedBase58();
        KLog.i("InitWalletManager","extendedBase58:" + extendedBase58);
        ...
        KLog.i("InitWalletManager","childExtendedBase58:" + childExtendedBase58);
        ...
        KLog.i("InitWalletManager","privateKey:" + privateKey);
        KLog.i("InitWalletManager","publicKey:" + publicKey);
        KLog.i("InitWalletManager","address:" + Constants.PREFIX_16 + address);

        return keyPair;
    }

    public Flowable<HLWallet> importPrivateKey(Context context, String privateKey, String password) {
        if (privateKey.startsWith(Constants.PREFIX_16)) {
            privateKey = privateKey.substring(Constants.PREFIX_16.length());
        }
